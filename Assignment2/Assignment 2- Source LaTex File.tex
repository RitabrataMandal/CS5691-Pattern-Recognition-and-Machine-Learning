...
\begin{solution}
	...
	\end{align*}
	By selecting a threshold of $0.7$, data points 1, 2, 3 and 4 are grouped into one cluster. But if we choose $0$ as a threshold then eigenvector  $e_{\lambda_2}$ cluster data points 1 \& 2 in one cluster and data points 3 \& 4 in another cluster.
\end{solution}
\end{parts}
...
\begin{solution}
	...
	\begin{align*}
		\lambda_2 \le \frac{n}{n-1}\bar{d} \le \lambda_n\quad\qed
	\end{align*}
\end{solution}
...
\begin{solution}
	...
	X^T X z = \beta z
	\end{align*}
	Premultiplying the second equation by $y^T X$, we get
	\begin{align*}
...
\begin{solution}
	As $XX^T$ and $X^TX$ are symmetric matrices. By Spectral Theorem we conclude that it will have orthonormal set of eigenvectors.\\
	...
	\begin{align*}
		XX^Tu_i=\lambda_iu_i\quad\forall~i\in\{1,2,\cdots,N\}
	\end{align*}
	By premultiplying with $X^T$ we get
	\begin{align*}
		X^TXX^Tu_i=\lambda_iX^Tu_i\Rightarrow X^TX \tilde{v}_i=\lambda_i\tilde{v}_i \quad\forall~i\in\{1,2,\cdots,N\}
	\end{align*}
	where $\tilde{v}_i=X^Tu_i \quad\forall~i\in\{1,2,\cdots,N\}$\\
	Now by normalizing we write 
	\begin{align*}
		v_i = \frac{\tilde{v}_i}{\norm{v_i}}=\frac{X^Tu_i}{\sqrt{u_i^TXX^Tu_i}}=\frac{X^Tu_i}{\sqrt{\lambda_i u_i^Tu_i}}=\frac{X^Tu_i}{\sqrt{\lambda_i}}\quad\forall~i\in\{1,2,\cdots,N\}
	\end{align*}
...
\begin{solution}
	Below are the given steps involved to compute the eigenvalues of $S$
	\begin{enumerate}[\textbf{Step-\arabic*:}]
		\item We compute the smaller matrix
		\begin{align*}
			S^{'}_{N\times N}=\frac{1}{N}X_{N\times D}X_{D\times N}^T
		\end{align*} 
		this step has a time complexity of $O(N^2D)$
		\item This step finds the eigenvalues $\lambda_i$ and the corresponding normalized eigenvectors $u_i$ that satisfies 
		\begin{align*}
			S^{'}u_i=\lambda_i u_i
		\end{align*}
		this step has a time complexity of $O(N^3)$
		\item Now computing the orthonormal eigenvectors of the matrix $S$ as follows(as computed before)
		\begin{align*}
			v_i=\frac{X^Tu_i}{\sqrt{\lambda_i}}\quad\forall~i\in\{1,2,\cdots,N\}
		\end{align*}
		this step has a time complexity of $O(N^2D)$
	\end{enumerate}
	Hence, time complexity can't be $O(N^3)$ it is $O(N^2D)$. Which is a big improvement form $O(D^3)$
%	This is a massive improvement over the original $O(D^3).$
\end{solution}
\end{parts}
...
\begin{solution} 
		...
		\begin{align*}
			\frac{\partial ^2 L}{\partial ^2 m} \Bigg{|}_{m_{+}}=\sqrt{(S_{xx} - S_{yy})^2 + 4S_{xy}^2}>0;\quad\frac{\partial ^2 L}{\partial ^2 m} \Bigg{|}_{m_{-}} = -\sqrt{(S_{xx} - S_{yy})^2 + 4S_{xy}^2}<0
		\end{align*}
